...
function Load(){//Метод срабатывает при нажатии на кнпопку "Загрузить"
    ...
    let AjaxGetBoolean = false;//Финальный флаг, разрешающий отправку Ajax запроса, нужен чтобы не отправлять путой массив
    let oFormData = new FormData();//Создаем форму для отправки в Ajax режиме
    oFormData.append('encryption_type',$('input[name="encryption_type"]:checked').val());//Передаем выбранный тип шифрования
    ...
    if (oFiles.length!=0){//Если выбран для загрузки хотя бы один файл
        ...
        if (FilesCheked(oFiles[index])==0){//Проверяем файл на тип и размер
                ...
                AjaxGetBoolean = true;//Добавлен хоть один элемент для запроса
            }else {
        ...
        if (AjaxGetBoolean){//Проверяем разрешена ли отправка
